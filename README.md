...
| --- | --- |
| `← →` 或 `A` `D` | 左右移动 |
| `↑` 或 `W` 或 `Space` | 跳跃（空中再按一次双段跳；贴墙下落时触发墙跳） |
| `J` | 剑斩攻击；连按最多 3 段连击，伤害递增 (×1.0 / ×1.15 / ×1.6) |
| `S` + `J` 在地面 | 滑铲（变低姿态、附带冲撞伤害） |
| `S` + `J` 在空中 | 下劈（高速下坠 + 落地范围 AOE） |
| `L` | 投掷手里剑（消耗一枚，可补充） |
| `K` | 防御（减伤 75% + 抵抗击退） |
| `Esc` | 暂停/继续 |
| `Space` / `Enter` | 标题界面开始 / 死亡后重生 |

### 胜负条件

- **胜利**：击败 BOSS（天狗将军）后抵达关卡终点旗帜
- **失败**：3 条命全部用完后进入 Game Over
- **复活**：仍有生命时，从最近激活的检查点重生（满血复活）

### 战斗反馈

- **连击系统**：连续命中累计 combo，HUD 中央闪现 "x2 COMBO" / "x3 COMBO"
- **打击感**：每次命中产生 ~70 ms hit-stop（物理与补间冻结）+ 火星粒子 + 飘伤害数字 + 屏幕震动
- **粒子效果**：跳跃/落地溅起灰尘、敌人受击红色血溅、连击 3 段大字号伤害弹出
- **无敌帧**：受伤后 800 ms 闪烁无敌
- **防御**：仅承受 25% 伤害，且抵抗击退
...
| 区域 | 范围 | 主题 | 特色 |
| --- | --- | --- | --- |
| 竹林 (Bamboo) | 0 → 2600 | 雾色青竹晨光 | 介绍跳跃节奏 |
| 庭院 (Courtyard) | 2600 → 5400 | 锦鲤池与樱花 | 移动平台、下落平台、墙跳 |
| 城堡 (Castle) | 5400 → 8000 | 红月之夜 | 攀爬挑战 + BOSS 战 |
| BOSS：天狗将军 | x ≈ 7700 | 三段血条 | Phase 1 砍击 / Phase 2 冲刺 / Phase 3 手里剑齐射 |

### 资源系统

- **金币 (Coin)**：+25 分；自动磁吸到玩家
- **团子 (Dango)**：恢复 30 HP
- **手里剑包 (Shuriken pickup)**：补充 3 枚弹药
- **酒桶 (Barrel)**：可破坏，35% 概率掉团子，65% 掉金币
- **石灯检查点 (Checkpoint)**：触碰后激活，作为下次重生点
