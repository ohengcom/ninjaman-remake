...
1. [x] 修复 canvas 比例。
2. [x] 修复 projectile 和状态 timer 高风险 bug。
3. [x] 修正 SP 里程碑和存档校验。
4. [x] 建资源 manifest。
5. [x] 替换 Player atlas 和 AnimationManager。
6. [ ] 替换 Enemy/Boss atlas。
7. [ ] 升级场景背景、平台、VFX。
8. [x] 建视觉回归。
9. [x] 分步升级 Vite、Vitest、TypeScript。
10. [x] 拆分 `GameScene` 和事件类型。
11. [x] 改善跳跃缓冲与输入手感 (Jump Buffer)。
